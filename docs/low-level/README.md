...
> **⚠️ Low-Level API**
>
> This is the original documentation for the plugin **prior to version 2025.16**. It describes the **low-level operations** that call CIC APIs directly. These operations are still shipped with the current plugin and remain the right choice when you need fine-grained control — but they require more setup, configuration, and per-call tuning on the user's side.
>
> If you are starting fresh, see the [main README](../../README.md) for the higher-level Web UI buttons, document model, and `CIC.*` operations introduced in 2025.16.
