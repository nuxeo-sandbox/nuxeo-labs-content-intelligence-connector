...
So, copy the whole contribution block (`<knowledgeEnrichment>`, `<knowledgeDiscovery>`, ...) from the plugin's own contribution — the matching `service-<family>-contrib.xml` — and paste it into your Studio project, then add or change what you need.

For example, to add the embeddings configuration to the `default` Knowledge Enrichment contribution:

**1.** Copy the whole `<knowledgeEnrichment>` block from the plugin's own contribution, `OSGI-INF/service-enrichment-contrib.xml`, and paste it into your Studio project.
...
> [!NOTE]
> * Contributions using a new name are not affected: this only applies when you reuse an **existing** name. A contribution introducing a **new** name (`tenantA`, `kd_emea_1`, ...) is self-contained: Hot Reload removes it and puts it back unchanged, with no side effect.
...
> [!IMPORTANT]
> The transaction is committed just *before* the event is fired. This means the data generated by the call to CIC is already persisted whatever the type of listener (inline, post-commit, synchronous or asynchronous): if your listener fails, the enrichment results remain, and there is no need to re-process the document(s).
...
#### Event Handler in your Nuxeo Studio project
...
* You may or may not check *Is asynchronous*: as explained above, if your code fails, the data generated by CIC is still there, since the transaction was committed before the event was fired.

**3.** Add the automation chain you need. Here is an example logging all the values. `input` is the first document of the batch.
...
Your listener can be declared with `postCommit="true"` or `"false"`, and with `async="false"` or `"true"`: it is up to you, depending on how you want to handle the event. In all cases, the data generated by the call to CIC is persisted, because the transaction was committed before the event was fired.

#### Other notes

A couple of behaviours to keep in mind when you write your handler:
